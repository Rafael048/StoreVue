<template>
    <v-dialog v-model="props.store.subTable">
        <v-card border>
            <v-text-field v-model="search" variant="outlined" class="ma-3" :label="`Buscar`" prepend-inner-icon="mdi-magnify">
               
            </v-text-field>
                <v-data-table :items="props.items" :headers="headers"  :search="search" items-per-page-text="Elementos" :header-props="{class : 'text-h6'}"
      >
